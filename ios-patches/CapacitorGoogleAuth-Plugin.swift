 ...
 *
 * Patched for GoogleSignIn 7.1 (see ios-release.yml ITMS-91061 fix):
 * GIDGoogleUser.authentication/.serverAuthCode were removed upstream.
 * Tokens now live on GIDGoogleUser.accessToken/.idToken/.refreshToken
 * (each a GIDToken with .tokenString), configuration is set as a
 * property on GIDSignIn instead of passed into signIn(with:...), and
 * serverAuthCode now arrives on GIDSignInResult from the interactive
 * signIn completion only (not on restorePreviousSignIn).
 */
@objc(GoogleAuth)
            ...
            if self.googleSignIn.hasPreviousSignIn() && !self.forceAuthCode {
                ...
                self.googleSignIn.signIn(withPresenting: presentingVc, hint: nil, additionalScopes: self.additionalScopes) { result, error in
                    ...
                    guard let result = result else {
                        self.signInCall?.reject("Sign in returned no result.");
                        return;
                    }
